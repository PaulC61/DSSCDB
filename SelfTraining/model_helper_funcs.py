...
import numpy as np
import pandas as pd
from sklearn.metrics import r2_score, mean_absolute_error, classification_report, ConfusionMatrixDisplay
from sklearn.mixture import GaussianMixture
import matplotlib.pyplot as plt
import seaborn as sns
...
def classification_results(clf_model, X_train, X_test, Y_train, Y_test, title="Default:"):
    pred_train = clf_model.predict(X_train)
    pred_test = clf_model.predict(X_test)
    
    print("Training Classification Report:")
    print(classification_report(Y_train, pred_train, labels=clf_model.classes_))
    print("Test Classification Report:")
    print(classification_report(Y_test, pred_test, labels=clf_model.classes_))

    fig, axes = plt.subplots(1,2, figsize=(15, 5))
    fig.suptitle(f'{title} Classification Model')

    # Training Set Results
    ConfusionMatrixDisplay.from_predictions(ax=axes[0], y_true=Y_train, y_pred=pred_train)
    axes[0].set_title(f"Train Set")

    # Test Set Results
    ConfusionMatrixDisplay.from_predictions(ax=axes[1], y_true=Y_test, y_pred=pred_test)
